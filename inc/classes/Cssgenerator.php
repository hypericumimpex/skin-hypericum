...
class WOODMART_Cssgenerator {
	...
	private function _get_options_from_section( $section ) {
		return array_filter( $this->_options, function($el) use($section) {
	...
	private function _render_option( $opt ) {
		?>
			<div class="css-checkbox" data-parent="<?php echo (isset($opt['parent'])) ? $opt['parent'] : 'none'; ?>">
				...
				<label for="<?php echo esc_attr($opt['id']); ?>"><?php echo esc_html($opt['title']); ?></label>
				<?php if ( isset( $opt['description'] ) ): ?>
					<div class="css-description"><?php echo esc_html($opt['description']); ?></div>
				<?php endif ?>
	...
	private function _get_children($id) {
		return array_filter( $this->_options, function($el) use($id) {
		...
		<div class="xtemos-loader-wrapper">
			<div class="xtemos-loader">
				<div class="xtemos-loader-el">
					<img src="<?php echo WOODMART_ASSETS . '/images/loader/loader-el-1.svg'; ?>" alt="">
				</div>
				<div class="xtemos-loader-el">
					<img src="<?php echo WOODMART_ASSETS . '/images/loader/loader-el-2.svg'; ?>" alt="">
				</div>
			</div>
			...
			<div class="css-file-information">
				...
				<table>
					...
					<tr>
						...
						<td>
							<strong><?php echo $theme->get('Version'); ?></strong>
						</td>
					</tr>
				</table>
	...
	public function process_form() {
		...
		if( ! isset( $_POST['generate-css'] ) || empty( $_POST['generate-css'] ) ) return;

		$data = $_POST['css-data'];

		if( ! $json = base64_decode($data) ) {
		...
		$response = wp_remote_get($this->_generator_url . '?generate_css=' . $data, array('timeout' => 30));
